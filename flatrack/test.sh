...
CONFIGS_DIR=$DOCKERFILES/configurations
...
# docker-compose -f $CONFIGS_DIR/common.yml run --rm $TEST_IMAGE sh -c "source /flatrack/test-helpers.sh && bash /tests/$TEST.sh"

# TODO: pass in env of TESTHELPERS as /flatrack/test-helpers.sh

docker-compose -f $CONFIGS_DIR/common.yml run --rm testrunner bash /dockerfiles/tests/$TEST.sh

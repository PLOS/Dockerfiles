...
class Test():
  ...
  def test_scraper_list(self, stack):
    ...
    scrapernames = str(self.d.containers.run('plos/ploscrawl:dev', 'scrapy list', detach=False, stderr=True, name='scrapy_list'))  #, auto_remove=True

    self.d.containers.get('scrapy_list').remove()
  ...
  def test_scrape_articles(self, stack):
    ...
    self.d.containers.get('scrapy_crawl').remove()
